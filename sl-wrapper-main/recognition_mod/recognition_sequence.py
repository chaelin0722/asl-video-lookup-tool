...
if __name__ == "__main__":
    folder_path = "/Users/zzenninkim/Documents/Research/sl-wrapper-main/segmentation_mod/time_txt"
    video_path = "/Users/zzenninkim/Documents/Research/sl-wrapper-main/clips"
    processing_times_path = "/Users/zzenninkim/Documents/Research/sl-wrapper-main/recognition_processing_times.txt"
    ...
    with open(processing_times_path, 'w') as time_file:
        for file_name in file_list:
            file_path = os.path.join(folder_path, file_name)

            # txt 파일로부터 시간 구간 읽어오기
            segments = read_time_segments(file_path)

            # 각 구간에 대해 sign recognition 수행
            video_file = os.path.join(video_path, file_name.split(".")[0] + ".mp4")
            start_time = time.time()
            recognition_results = recognize_segments_in_video(video_file, segments)
            end_time = time.time()

            # 처리 시간 계산 및 기록
            processing_time = end_time - start_time
            time_file.write(f"{file_name}: {processing_time:.2f} seconds\n")
            print(f"Processing time for {file_name}: {processing_time:.2f} seconds")

            # JSON 파일로 결과 저장
            output_json_path = os.path.join("/Users/zzenninkim/Documents/Research/sl-wrapper-main/results", file_name.split(".")[0] + ".json")
