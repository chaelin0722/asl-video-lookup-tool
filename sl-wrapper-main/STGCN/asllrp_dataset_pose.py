...
class ASLLRP(data_utl.Dataset):
    def __init__(self, datadir, video_file=None, gloss_dict=None, transforms=None, pose_map_file=None):
        ...
        with open(video_file, 'r') as file:
            ...
            for row in reader:
                ...
                self.pose_paths.append(pose_map_file + row[0].split(".")[0] + ".npy")
                self.video_info.append(row)

                g = row[1].strip()
                self.labels.append(self.gloss_dict[g]) # KeyError: '5notknowwhattodo'
    ...
    def __getitem__(self, index):
        ...
        name_dict = {"test": name[0], 'filename': name[0], 'gloss': name[1]}
