...
export default {
  ...
  methods: {
    ...
    clearFrames() {
      ...
      // 배경 제거
      if (this.selectionEl) {
        this.selectionEl.style.background = 'none';
      }
    },
    updateFrames(frames) {
      ...
      this.setupBackground(); // redraw frames
    },
    updateTickPosition(position) {
      this.position = position; // reflect current video playback position
    ...
    setupBackground: function () {
      ...
      // set background according to frames
      ...
      for (let i = 0; i < this.frameCount; i++) {
        ...
        img.onload = () => {
          ...
          // set background when last frame is rendered
...
#selection {
  ...
  border-top: $b solid  rgba(0, 123, 255, 0.8);
  border-bottom: $b solid rgba(0, 123, 255, 0.8);
  border-radius: $br;
  z-index: 1;
